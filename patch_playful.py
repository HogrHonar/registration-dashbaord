...
# Replace the preReg stats
old_prereg = """<div className="space-y-4">
                  {data?.preRegDepartmentStats.map((dept, idx) => (
                    <div key={idx} className="border rounded-lg p-4">
                              ...
                              style={{
                                width:
                                  dept.totalRequests > 0
                                    ? (dept.qualified / dept.totalRequests) * 100
                                    : 0,
                              }}
                            />
                          </div>
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {dept.totalRequests > 0
                          ? ((dept.qualified / dept.totalRequests) * 100).toFixed(1)
                          : 0}
                        % ئەگەری وەرگرتن هەیە
                      </p>
                    </div>
...
new_prereg = """<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {data?.preRegDepartmentStats.map((dept, idx) => {
...
# Replace the nextStep stats
old_nextstep = """<div className="space-y-4">
                  {data?.nextStepDepartmentStats.map((dept, idx) => (
                    <div key={idx} className="border rounded-lg p-4">
                      <div className="flex justify-between items-center mb-2">
                        <span className="font-semibold">{dept.name}</span>
                        <span className="text-sm text-gray-500">
...
new_nextstep = """<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {data?.nextStepDepartmentStats.map((dept, idx) => {
                    const percent = dept.totalRequests > 0 ? (dept.qualified / dept.totalRequests) * 100 : 0;
                    const dashArray = 2 * Math.PI * 36;
                    const dashOffset = dashArray - (dashArray * percent) / 100;
...
code = code.replace(old_prereg, new_prereg)
code = code.replace(old_nextstep, new_nextstep)
